...
@tool
def pod_not_attempted_today_tool(
    college_name: Optional[str] = None,
    limit: int = 20,
) -> list[dict]:
...
def pod_pass_fail_summary_tool(
    ...
    return analytics.get_pass_fail_summary(
...
@tool
def pod_difficulty_breakdown_tool(
    college_name: Optional[str] = None,
) -> list[dict]:
...
@tool
def pod_language_breakdown_tool(
    college_name: Optional[str] = None,
) -> list[dict]:
...
def pod_active_streaks_tool(
    ...
    return analytics.get_active_streaks(
        college_name=college_name, min_streak=min_streak, limit=limit
    )


@tool
def pod_lost_streak_tool(
    college_name: Optional[str] = None,
    limit: int = 20,
) -> list[dict]:
...
@tool
def pod_total_points_today_tool(
    college_name: Optional[str] = None,
) -> list[dict]:
...
def pod_top_scorers_tool(
    ...
    return analytics.get_top_scorers(college_name=college_name, limit=limit)


@tool
def pod_badge_earners_tool(
    college_name: Optional[str] = None,
    limit: int = 20,
) -> list[dict]:
...
@tool
def pod_weekly_badge_earners_tool(
    college_name: Optional[str] = None,
    limit: int = 20,
) -> list[dict]:
