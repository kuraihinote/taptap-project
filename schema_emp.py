...
- Domain names in public.domains are free-text and user-created — they may not match
  how faculty refer to them. Split the faculty's term into broad keyword fragments
  and combine with OR: (d.domain ILIKE '%word1%' OR d.domain ILIKE '%word2%')
  rather than matching the full phrase exactly.
...
3. PASS RATE BY DOMAIN (e.g. DSA, Python, Algorithms):
SELECT
    d.domain,
    COUNT(ets.id)                                                          AS total_submissions,
...
JOIN public.user u ON u.id = ets.user_id
WHERE u.role = 'Student'
AND (d.domain ILIKE '%keyword1%' OR d.domain ILIKE '%keyword2%')
GROUP BY d.domain
